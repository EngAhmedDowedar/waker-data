"""Find the null integer field: disasm ngHttpClient::HandleUpdate(int) (crash at
+140 calling ngInteger::IntValue on a null node) + ngInteger::IntValue.
The field KEY string loaded right before the IntValue call is the culprit."""
import os, struct
...
def disfn(name=None,va=None,L=None,label=''):
    ...
    if not va: print(f"[{name} MISSING] (trying nearest)"); return
    ...
    for ins in md.disasm(code,start):
        ...
        mark = '  <==CRASH(+140)' if ins.address-start in (0x88,0x8a,0x8c,0x8e,0x84,0x86) else ''
        print(f'  0x{ins.address:08x}: {ins.bytes.hex():<8} {mn:<7} {op}{an}{mark}')

for cand in ['_ZN12ngHttpClient12HandleUpdateEi','_ZN11CHttpClient12HandleUpdateEi']:
    if cand in symbols: disfn(cand, label=cand)
# IntValue
for cand in ['_ZN9ngInteger8IntValueEv','_ZNK9ngInteger8IntValueEv']:
    if cand in symbols: disfn(cand, L=0x30, label=cand)
# Fallback: dump any HandleUpdate symbols
print("\n=== HandleUpdate symbols ===")
for nm,(va,sz) in symbols.items():
    if 'HandleUpdate' in nm or 'IntValue' in nm: print(f"  0x{va&~1:08x} sz=0x{sz:x} {nm}")
