"""Fully reverse CLoadingScreen::ParseLastLoginPlayerInfo (crash +154) and the
OnReceiveResponse(0x80) call site that feeds it. Extract every JSON key + type.
Thunk map: 0x69300c -> ngInteger::IntValue (int field), 0x692fcc -> string get."""
import os, struct
...
THUNK={0x69300c:'IntValue[INT]',0x692fcc:'str_dup[STR]',0x692fdc:'Str.assign',0x69840c:'?',}
...
def disfn(name=None,va=None,L=None,label=''):
    ...
    for ins in md.disasm(code,start):
        ...
        if op.endswith('[r0, #0x40]') or op.endswith('[r1, #0x40]') or '#0x40]' in op: an+='  (GetNode vtbl)'
        print(f'  0x{ins.address:08x}: {ins.bytes.hex():<8} {mn:<7} {op}{an}')

# ParseLastLoginPlayerInfo
for c in ['_ZN14CLoadingScreen24ParseLastLoginPlayerInfoEPv']:
    if c in symbols: disfn(c, label=c)
    else: disfn(va=0x490418, L=0x140, label='ParseLastLoginPlayerInfo @0x490418')
# OnReceiveResponse around +876 (0x48fe20+0x36c=0x49018c) -> how lastLoginPlayer node obtained
disfn(va=0x49014c, L=0x60, label='OnReceiveResponse +0x32c..+0x38c (lastLoginPlayer call site)')
print("\n=== Parse* / LastLogin symbols ===")
for nm,(va,sz) in symbols.items():
    if 'LastLogin' in nm or 'ParsePlayer' in nm: print(f"  0x{va&~1:08x} sz=0x{sz:x} {nm}")
