...
def cstr(va,m=34):
    ...
    return s.decode('ascii','replace') if s and 2<=len(s)<=32 and all(32<=b<127 for b in s) and any(chr(b).isalpha() for b in s) else None
mT=Cs(CS_ARCH_ARM,CS_MODE_THUMB); mT.detail=True
def disfn(name,L=None):
    v,szz=sbn[name]; base=v&~1; o=va2off(base); ln=L or szz; litreg={}
    ...
    for ins in mT.disasm(data[o:o+ln],base):
        ...
        if mn in('add','adds') and len(parts)==3 and parts[1] in litreg:
            V=litreg[parts[1]]; Vs=V-0x100000000 if V&0x80000000 else V; s=cstr((0x75ab20+Vs)&0xFFFFFFFF)
            if s: an+=f' KEY="{s}"'
        if '#0x14]' in op: an+=' <arr.begin>'
        if '#0x8]' in op and mn=='ldr': an+=' <hasNext>'
        if '#0xc]' in op and mn=='ldr': an+=' <next>'
        if mn in('bl','blx') and ins.operands and ins.operands[-1].type==2:
            an+=' -> '+near(ins.operands[-1].value.imm&~1)
        print(f"  +0x{ins.address-base:03x} {mn:<6} {op}{an}")
disfn('_ZN15CFightingScreen13ParseFightersEPv')
disfn('_ZN15CFightingScreen20RequestRandomFighterEv')
