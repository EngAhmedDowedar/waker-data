...
def near(va):
    ...
    a,z,nm=syms[i]; return f"{nm}+0x{va-a:x}" if va-a<0x2000 else hex(va)
...
disfn('_ZN20ngGameMapSpriteLayer12HandleRenderEP10ngGraphics', 0x60)
# list ngGameMapSpriteLayer + mission/fight map-object methods
print("\n--- ngGameMapSpriteLayer / map-object add methods ---")
for a,z,nm in syms:
    if ('ngGameMapSpriteLayer' in nm or 'ngGameMap' in nm) and any(k in nm for k in ['Add','Insert','Push','Create']):
        print(f"  0x{a:08x} sz=0x{z:x} {nm}")
print("--- CCityMap / mission-target map methods ---")
for a,z,nm in syms:
    if 'CCityMap' in nm and not nm.startswith(('_ZTV','_ZTI','_ZTS')):
        print(f"  0x{a:08x} sz=0x{z:x} {nm}")
