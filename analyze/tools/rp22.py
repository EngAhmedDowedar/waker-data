...
def follow(t):
    ...
    try:
        ...
        if ins and ins[0].mnemonic=='ldr' and 'ip, [pc' in ins[0].op_str:
            word=struct.unpack_from('<I',data,va2off(base+8))[0]; return near((word+base+12)&0xFFFFFFFE)+'[ven]'
    except: pass
    return near(t&~1)
md=Cs(CS_ARCH_ARM,CS_MODE_THUMB); md.detail=True
va=0x5918c8; ln=0x80; o=va2off(va)
print(f"file_off(0x5918c8)=0x{o:x}")
litreg={}
for ins in md.disasm(data[o:o+ln],va):
    op=ins.op_str; mn=ins.mnemonic; an=''; parts=[p.strip() for p in op.split(',')]; rd=parts[0] if parts else ''
    ...
    if mn.startswith('b') and mn not in('bl','blx','bic') and ins.operands and ins.operands[-1].type==1:
        an+=f' => +0x{(ins.operands[-1].value.imm-va):x}'
    fo=va2off(ins.address)
    print(f"  +0x{ins.address-va:03x} f@{fo:x} {ins.bytes.hex():<8} {mn:<7} {op}{an}")
