...
for i in range(ds['size']//16):
    ...
    if nm and v: syms.append((v&~1,szz,nm,v&1)); sbn[nm]=(v,szz)
...
PIC=0x75ab20; mT=Cs(CS_ARCH_ARM,CS_MODE_THUMB); mT.detail=True; mA=Cs(CS_ARCH_ARM,CS_MODE_ARM); mA.detail=True
out=open('rp30_out.txt','w')
def disfn(name,ln=None):
    if name not in sbn: out.write(f"[{name} MISSING]\n"); return
    v,szz=sbn[name]; base=v&~1; thumb=(v&1)==1; md=mT if thumb else mA; o=va2off(base); L=ln or szz or 0x100
    out.write(f"\n===== {name} 0x{base:08x} [{'T' if thumb else 'A'}] =====\n")
    litreg={}
    for ins in md.disasm(data[o:o+L],base):
        ...
        if mn in('add','adds','add.w') and len(parts)==3 and parts[1] in litreg:
            ...
            if s: an+=f' STR="{s}"'
        if '#0x5c]' in op: an+=' <crashfield 0x5c>'
...
for cand in ['_ZN20CMarketCateCellRender8SetValueEPvhii','_ZN16CMarketCateScreen13InitCateTableEv','_ZN16CMarketCateScreen11ViewDidLoadEv']:
    disfn(cand)
# also search names containing MarketCate
out.write("\n--- MarketCate symbols ---\n")
for a,z,nm,th in syms:
    if 'MarketCate' in nm: out.write(f"  0x{a:08x} sz=0x{z:x} {nm}\n")
out.close(); print("OK")
