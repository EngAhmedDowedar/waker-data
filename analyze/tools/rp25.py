...
ds=sec['.dynsym']; dt=sec['.dynstr']; symv={}; sbn={}
for i in range(ds['size']//16):
    ...
    if nm and v: symv[v&~1]=nm; sbn[nm]=(v,szz)
def near(va):
    nv=None
...
def disfn(name,L=None):
    v,szz=sbn.get(name,(0,0))
    if not v: print(f"[{name} MISSING]"); return
    start=v&~1; ln=L or szz or 0x140; o=va2off(start)
...
disfn('_ZN23CPropertyListCateScreen17OnReceiveResponseEiPv', L=0xb0)
disfn('_ZN6CHouse8GetPriceEv', L=0x20)
