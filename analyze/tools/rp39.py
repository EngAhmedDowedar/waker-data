...
def cstr(va,m=40):
    ...
    return s.decode('ascii','replace') if s and 2<=len(s)<=34 and all(32<=b<127 for b in s) and any(chr(b).isalpha() for b in s) else None
mT=Cs(CS_ARCH_ARM,CS_MODE_THUMB); mT.detail=True
def disfn(name):
    v,szz=sbn[name]; base=v&~1; o=va2off(base); ln=szz
...
disfn('_ZN12CServerMnger15ParseServerListEPv')
